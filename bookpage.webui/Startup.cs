...
namespace bookpage.webui
{
    public class Startup
    {
        ...
        }//BUNUN ÜSTÜNDEN appsettingsteki host port vb bilgilere ulaşıcaz
        ...
        //Email Gönderme Servisi
            services.AddScoped<IEmailSender,SmptpEmailSender>(i=> new SmptpEmailSender(
            _Configuration["EMailSender:Host"],//emailsender altındaki host bilgisini aldım 
            _Configuration.GetValue<int>("EMailSender:Port"),//portuda aldım ama int diye bi dönüştürme uyguladım
...
//bunları ekledimki klasörün içindekileride tanısın yoksa applicationcontexti falan uygulama klasörden tanımaz buradan tanır. Servisleri ekledik kullan dediğimiz yer ise routenin hemen üstüdür.
   ...
   //         options.User.AllowedUserNameCharacters="";//usernamenin içerisinde olmasını istediğin karakterleri yazabilirsin.
            ...
            options.SignIn.RequireConfirmedEmail=true;// kullanıcı üye olur ve mutlaka hesabını onaylamalı
     ...
     //uygulama sen login olunca benzersiz sayı üretir ve tarayıcına bırakır o ziyaret ettiğin sayfada server tarafındada sesion oluşturulur. Sessin nedir global değişken cookie ile session birbiriyle iletişim halinde.       
            services.ConfigureApplicationCookie(options=>{
              ...
              options.Cookie=new CookieBuilder{
               ...
               SameSite=SameSiteMode.Strict//bu durumda b kullanıcısı akullanıcısının coockiesine sahip olsa bile adres önemli güvenlik için crosside atackları engelliyoruz
              };
